...
def parse_request(req: str) -> ct.HttpRequestDetails:
    """
    ...
    :rtype: HttpRequestDetails
    :raises HttpRequestError: If the request is not well-formed.
    """
    ...
    if len(request_line) != 3:
        raise ex.HttpRequestException('Invalid request header length')

    # Get HTTP method
    http_method: str = request_line[0]
    if http_method not in http_methods:
        raise ex.HttpRequestException(f'Invalid HTTP method: {http_method}')
    ...
    if not request_line[2].startswith('HTTP/'):
        raise ex.HttpRequestException('Invalid HTTP version')
    ...
    for hdr_part in header_parts:
        if len(hdr_part) != 2:
            raise ex.HttpRequestException('Invalid header format')
    ...
    if empty_line_index != -1:
        try:
            ...
            raise ex.HttpRequestException('Invalid JSON body')

    return ct.HttpRequestDetails(http_method, path, version_dict, headers_dict, parsed_body)
